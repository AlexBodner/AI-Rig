...
Outcome in one line. Writing `s := T − t`, the nonlinear balance forces `α = 1 − β` (B4), the viscous balance and Leray's lower bounds force `β ≤ 1/2` (B5), finite energy forces `β ≥ 2/5` (B6), and `β = 1/2` is killed by NRS/Tsai/Chae–Wolf and independently by ESS (B3, B9), so **the only surviving self-similar blowup window for the Clay problem is `2/5 ≤ β < 1/2`, i.e. Type II with an Euler self-similar leading profile and formally (but only singularly) perturbative viscosity** (B7). The same arithmetic in dimension `d` with dissipation `−ν(−Δ)^σ` gives the window `2/(d+2) ≤ β ≤ 1/(2σ)`, which is empty exactly when `σ > (d+2)/4` — the Lions exponent, `5/4` at `d = 3` and `1` at `d = 2` (B10). The line stops (Breakpoint) at the fact that no smooth decaying Euler self-similar profile with `β` in the surviving window is known to exist, and that the viscous correction, though formally `O(s^{1−2β})`, multiplies the highest-order operator and is therefore a singular perturbation.
...
Summary of statuses: B1, B2, B4, B5, B6, B8, B10 PROVED-HERE (B1, B2, B8, B10 additionally COMPUTED); B3 PROVED-HERE under three explicitly stated hypotheses (D0)–(D2), with the decay lemma itself KNOWN (not reproved); B7 PROVED-HERE for the arithmetic and CONJECTURE for the perturbativity claim; B9 KNOWN; one FAILED sub-item is recorded inside B5.
...
At `ν = 1` this is `Res = 0` with `Res` as in "Setting and notation", and it is the equation quoted in no-go item 2.3 with the opposite overall sign. **COMPUTED** (script Part 1, run with symbolic `α, β` and a random exact-rational profile; see B4 for the general-exponent table): the residual of the substituted equation against the claimed profile form is `0` after imposing `α = 1 − β`, and nonzero before.
...
**What Theorem B3 is not.** It is *weaker* than NRS and Tsai, and the difference is exactly the interesting part. My hypotheses (D1)–(D2) are decay hypotheses on `Π`; because `Π` contains the term `(1/2) y·U`, (D2) needs roughly `|U(y)| = o(|y|^{-4})`, which `U ∈ L^3` does not give directly. **KNOWN, not reproved here:** the mathematical content of NRS 1996 [checked via WebSearch: Acta Math. 176 (1996) 283–294] and Tsai 1998 is precisely the missing lemma — that `U ∈ L^3(R^3)` (NRS), or the local energy bounds of a suitable weak solution (Tsai), force enough decay of `U` and `P` for the maximum-principle argument to run. I cannot reconstruct their intermediate decay statement from memory and do not guess it. So: the *identity* and the *shape* of the Liouville argument are reproduced here in full; the *decay bootstrap* is cited.

**KNOWN strengthening [checked via WebSearch].** D. Chae, J. Wolf, "On the Liouville type theorems for self-similar solutions to the Navier–Stokes equations", Arch. Rational Mech. Anal. 225 (2017) 549–572, generalize NRS and Tsai and, using their Liouville theorem, exclude asymptotically self-similar blowup with profile in `L^{p,∞}(R^3)` for `p > 3/2`. This matters here: the *matched* profile tail is `|U(y)| ~ |y|^{-α/β}`, which at `β = 1/2` is `|y|^{-1} ∈ L^{3,∞}` but not in `L^3` — exactly the case NRS's hypothesis misses and Chae–Wolf covers. So `β = 1/2` is dead even for the physically natural (matched, non-`L^3`) profile, and asymptotically self-similar `β = 1/2` is dead too. Independently, ESS (2.4) kills `β = 1/2` because `‖u(t)‖_{L^3} = ‖U‖_{L^3}` is constant in `t` (see the table in B10), contradicting `lim_{t→T*}‖u(t)‖_{L^3} = ∞`; the ESS route needs `U ∈ L^3`, the Chae–Wolf route does not.
...
**B4.2 (rigidity) — PROVED-HERE.** With `α = 1 − β` the transformed equation reads
...
**B5.2 (Leray's lower bound; KNOWN input 2.5) — PROVED-HERE given 2.5.** For a nontrivial bounded profile, `‖u(t)‖_{L^∞} = s^{-α}‖U‖_{L^∞}` with `α = 1 − β < 1/2`, so `‖u(t)‖_{L^∞} = o(s^{-1/2})`, contradicting `‖u(t)‖_{L^∞} ≥ c s^{-1/2}` (2.5). The same contradiction comes from `‖∇u(t)‖_{L^2} = s^{(3β-2)/2}‖∇U‖_{L^2}` against `‖∇u(t)‖_{L^2} ≥ c s^{-1/4}`, which needs `(3β−2)/2 ≤ −1/4`, i.e. `β ≤ 1/2` (table in B10). This kills asymptotically self-similar `β > 1/2` too, not just the exact ansatz.

**B5.3 (CKN; KNOWN input 2.6) — PROVED-HERE given 2.6.** On the parabolic cylinder `Q_r = B_r × (T*−r^2, T*)` at the singular point, `r^{-1}∫∫_{Q_r}|∇u|^2 dx dt ~ r^{6β−3}` in the regime `β > 1/2` (B10 table, where the exponent is derived and is piecewise), which tends to `0` for `β > 1/2`; CKN `ε`-regularity then says the point is regular. Again this covers the asymptotic case.
...
Hence **`β ≥ 2/5` is forced by finite energy** in `d = 3`. This argument does not need `U ∈ L^2`: if instead `U` has the generic matched tail `|U(y)| ~ |y|^{-α/β}` (so that `u(x,T*) ~ |x|^{-α/β}` is `s`-independent, which is what "matching to the outer solution" means), then `∫_{|x| ≤ r_0}|u|^2 dx = s^{(d+2)β−2}∫_{|y| ≤ r_0 s^{-β}}|U|^2 dy ~ s^{(d+2)β−2} (s^{-β})^{d − 2α/β}`, and the two powers cancel identically, giving a finite `s`-independent answer precisely when `d − 2α/β > 0`, i.e. `2(1−β)/β < 3`, i.e. `β > 2/5` again. The threshold is the same either way, which is the arithmetic reason `2/5` is not an artifact of assuming `U ∈ L^2`.

**B6.2 (the endpoint) — CONJECTURE.** At `β = 2/5` exactly, a fixed positive amount of energy sits in a ball of radius `s^{2/5} → 0`, i.e. energy concentrates at the singular point. With the generic matched tail `|U| ~ |y|^{-α/β} = |y|^{-3/2}` the inner integral diverges logarithmically, `∫_{|y|<Y}|y|^{-3} dy ~ log Y`, giving `∫_{|x|≤r_0}|u|^2 ~ log(1/s) → ∞`, which is again a contradiction; so strictly `β > 2/5` for matched profiles. Only the case `U ∈ L^2` with exactly `β = 2/5` survives this argument, and excluding it needs a non-concentration statement for suitable weak solutions (no atom of `|u|^2` at a first singular time), which I have not proved and do not find stated in the no-go list. I record `β = 2/5` as the (probably excludable) endpoint, and use the closed window `2/5 ≤ β < 1/2` below to stay on the safe side.
...
**B7.1 (what survives) — PROVED-HERE, given the KNOWN inputs cited.** Combining B4.1 (`α = 1 − β`), B5 (`β ≤ 1/2`, three independent proofs), B6 (`β ≥ 2/5`), and B3/2.3/2.4 (`β = 1/2` excluded, i.e. Type I is dead): a self-similar blowup of 3D Navier–Stokes in the Clay class must have
...
### B8. Discretely self-similar profiles: the identity and why the maximum principle dies — **PROVED-HERE** and **COMPUTED**
